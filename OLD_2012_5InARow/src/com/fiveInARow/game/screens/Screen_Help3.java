...
public class Screen_Help3  extends Screen {
	
	public Screen_Help3(IGame game, Activity activity) {
		super(game, activity);
	}
		...
		for (int i = 0; i < len; i++) {
			...
			if (event.type == TouchEvent.TOUCH_UP) {
				if (TouchAndCoordinatesUtils.inBoundOfSquareArea(event, TouchAndCoordinatesUtils.squareArea_nextOnHelpScreens)) {
					this.m_Game.setScreen(new Screen_Help4(this.m_Game, this.m_Activity));
	...
	public void present(float deltaTime) {
		...
		g.drawPixmap(Assets.img_help3, 0, 0);
	}

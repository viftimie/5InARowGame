package missingInAndroid;

/*
 * > Exact copy of javax.swing.event.EventListenerList
 * > Because the SWING part is absent from Android's Java, and EventListenerList class has no other dependencies.
 */
...
public class EventListenerList implements Serializable {
	private static final long serialVersionUID = 8499386864464843836L;
	...
	public synchronized <T extends java.util.EventListener> void remove(
		...
		if (position >= 0) {
			...
			System.arraycopy(listenerList, position + 2, newList, position, listenerList.length - position - 2);
	...
	public String toString() {
		...
		for (int i = 0; i < listenerList.length; i += 2) {
			str += " type " + ((Class) listenerList[i]).getName() + " listener " + listenerList[i + 1].toString();
		}
		return str;
	}
